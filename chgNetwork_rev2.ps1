...
$num = $ESXIs.Count 
 for ( $i= 1; $i -le $ESXIs.Count; $i++)
 {
    $now = $ESXIs[$i].Name
    Get-NetworkAdapter -VM $ESXIs[$i].name | Select-Object Name,MacAddress,NetworkName,Type | export-csv F:\PoCLI\test.csv -Append
    Write-Progress -Activity "generate" -Status "Progressing Now:   $now   Total:   $i/$num " -PercentComplete($i / $ESXIs.Count*100 )
 }
